...
Deno.serve(async (req) => {
    try {
        ...
        console.log('[Scheduled Backup] Starting automated full backup...');

        // קבלת Access Token ל-Google Drive דרך service role
        const { accessToken: driveAccessToken } = await base44.asServiceRole.connectors.getConnection("googledrive");

        // שלב 1: אחזור כל הנתונים מכל הישויות
        console.log('[Scheduled Backup] Fetching all entities data...');
        ...
        console.log(`[Scheduled Backup] Fetched ${events.length} events, ${eventServices.length} event services`);
        ...
        // שלב 3: בניית גיבויים מועשרים לכל אירוע
            ...
            const enrichedEventServices = eventEventServices.map(es => {
                ...
                if (es.supplier_ids) {
                    try {
                        ...
                        console.warn(`[Scheduled Backup] Failed to parse supplier data for EventService ${es.id}`);
                    }
                }
        ...
        // שלב 4: גיבוי הצעות מחיר HTML
        const htmlQuotes = {};
        let htmlBackupCount = 0;
        const agreementsWithUri = signedAgreements.filter(a => a.agreement_content_uri);
        console.log(`[Scheduled Backup] Found ${agreementsWithUri.length} agreements with HTML to backup`);
                ...
                const results = await Promise.allSettled(
                    batch.map(async (agreement) => {
                        try {
                            ...
                            console.warn(`[Scheduled Backup] Failed to fetch HTML for agreement ${agreement.id}: ${e.message}`);
                            return null;
                        }
                    })
                );
        ...
        console.log(`[Scheduled Backup] Collected ${htmlBackupCount} quote HTML files`);
        ...
        const consolidatedBackup = {
            ...
            backup_by: 'scheduled_automation',
        ...
        console.log(`[Scheduled Backup] Consolidated backup file: ${backupFileName} (${(backupJson.length / 1024 / 1024).toFixed(2)} MB)`);

        // שלב 6: מציאת או יצירת תיקייה ראשית ב-Drive
        const mainFolderName = 'Base44 Backups';
        const mainFolderId = await findOrCreateFolder(driveAccessToken, mainFolderName, null);
        console.log(`[Scheduled Backup] Main folder ID: ${mainFolderId}`);

        // שלב 7: העלאת קובץ יחיד ל-Google Drive
        ...
        console.log(`[Scheduled Backup] Uploaded consolidated backup: ${uploadResult.id}`);
        ...
        try {
            await base44.asServiceRole.integrations.Core.SendEmail({
                to: SYSTEM_CREATOR_EMAIL,
                subject: `גיבוי אוטומטי הושלם - ${backupTimestamp}`,
                body: `
                    <div dir="rtl" style="font-family: Arial, sans-serif;">
                        <h2>גיבוי אוטומטי הושלם בהצלחה!</h2>
            ...
            console.warn('[Scheduled Backup] Failed to send notification email:', emailError.message);
        }

        console.log('[Scheduled Backup] Backup process completed successfully');
...
async function applyFIFOPolicy(accessToken, mainFolderId, maxBackups) {
    ...
    console.log(`[Scheduled Backup FIFO] Found ${files.length} backup files, max allowed: ${maxBackups}`);
    ...
    if (allBackupItems.length > maxBackups) {
        ...
        for (const item of itemsToDelete) {
            console.log(`[Scheduled Backup FIFO] Deleting old backup: ${item.name} (${item.type})`);
        ...
        console.log(`[Scheduled Backup FIFO] Deleted ${itemsToDelete.length} old backups`);
    }
}