...
async function generateQuoteHtml(eventId, base44Instance) {
    ...
    // New logic to calculate total based on new package structure
    ...
    if (event.all_inclusive && event.all_inclusive_price) {
        ...
        totalCostWithoutVat = servicesToSum.reduce((sum, s) => {
            // For main package items, custom_price holds the package price
            const price = parseFloat(s.custom_price) || 0;
            const quantity = parseFloat(s.quantity) || 1;
            let serviceTotal = price * quantity; // Quantity applies to main package item or standalone
