                ...
                if (missingTemplate && eventData.status !== 'in_progress') {
                    ...
                    if (hasMissing) {
                        ...
                        if (missingServices.length === 1) {
                            ...
                            contextData = { event_name: eventData.event_name || '', family_name: eventData.family_name || eventData.event_name || '', event_date: formatDate(eventData.event_date), event_contacts: formatEventContacts(eventData), service_name: ms.serviceName, min_suppliers: ms.minRequired, current_suppliers: ms.approvedCount, missing_count: 1, event_id: eventData.id };
                        } else {
                            const servicesList = missingServices.map(ms => `• ${ms.serviceName} (${ms.approvedCount}/${ms.minRequired})`).join('\n');
                            contextData = { event_name: eventData.event_name || '', family_name: eventData.family_name || eventData.event_name || '', event_date: formatDate(eventData.event_date), event_contacts: formatEventContacts(eventData), service_name: '', missing_count: missingServices.length, event_id: eventData.id };
                ...
                if (paymentTemplate) {
                    ...
                    if (balance > 0) {
                        ...
                        for (const { user: clientUser, phone } of clientUsers) {
                            ...
                            if (exists) continue;
                            const contextData = { event_name: eventData.event_name || '', family_name: eventData.family_name || eventData.event_name || '', event_date: formatDate(eventData.event_date), event_contacts: formatEventContacts(eventData), balance: formatCurrency(balance), event_id: eventData.id, client_name: clientUser.full_name || '' };
