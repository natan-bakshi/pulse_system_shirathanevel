...
import { verifyLog } from "../../shared/storedCardProvider.ts";
...
export default async function(req) {
  try {
    ...
    if(payment.agreement_id){
      if(payment.agreement_verified){
        await afterAgreementChange(base44.asServiceRole,payment.event_id);
        return Response.json({received:true});
      }
      if(!successful) return Response.json({received:true,pending_verification:true});
      const environment=payment.agreement_environment;
      if(!["qa","production"].includes(environment))throw new Error("Unknown payment environment");
      const key=secrets.get(environment==="qa"?"INVOICE4U_API_TOKEN_QA":"INVOICE4U_API_TOKEN");
      if(!key)throw new Error("Verification key unavailable");
      await verifyLog({environment,key,purpose:"hosted"},{paymentId:String(data.PaymentId||""),traceId:String(data.ClearingTraceId||""),amount:Number(payment.amount)+Number(payment.processing_fee_amount||0),currency:payment.currency},payment.created_date);
      const claim=await base44.asServiceRole.entities.Payment.updateMany({id:payment.id,payment_status:"pending",agreement_verified:false,agreement_callback_busy:{$ne:true}},{$set:{invoice4u_clearing_status:"verifying",agreement_callback_busy:true}});
      if(claim.updated!==1)return Response.json({received:true});
    }
    const update = { ...(payment.agreement_id?{agreement_verified:true}:{}), payment_status: successful ? "completed" : "failed", invoice4u_clearing_status: successful ? "approved" : (data.ErrorMessage || "declined"), invoice4u_payment_id: data.PaymentId || "", invoice4u_document_number: data.DocumentNumber || "", customer_name_on_card: data.CustomerName || "", client_ip: data.ClientIp || "", auth_number: String(data.AuthNumber || data.ConfirmationNumber || ""), card_suffix: String(data.CardSuffix || data.CardNumber || "").slice(-4) };
    ...
    if(payment.agreement_id)await afterAgreementChange(base44.asServiceRole,payment.event_id);
