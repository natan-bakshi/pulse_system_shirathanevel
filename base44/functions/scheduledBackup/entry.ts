...
Deno.serve(async (req) => {
    try {
        ...
        // אוטומציה מתוזמנת - אין משתמש מחובר, משתמשים ב-service role
        console.log('[Scheduled Backup] Starting automated daily backup...');

        // קבלת Access Token ל-Google Sheets ו-Google Drive
        const { accessToken: sheetsAccessToken } = await base44.asServiceRole.connectors.getConnection("googlesheets");
        ...
        console.log('[Scheduled Backup] Fetching data...');
        ...
        console.log(`[Scheduled Backup] Found ${allCategories.length} service categories:`, allCategories);
        ...
        console.log(`[Scheduled Backup] Built ${rows.length} rows (including header)`);

        // שלב 5: יצירת גיליון Google Sheets חדש
        const now = new Date();
        const sheetTitle = `גיבוי יומי - ${now.toLocaleDateString('he-IL')}`;
        
        const spreadsheet = await createSpreadsheet(sheetsAccessToken, sheetTitle);
        
        if (!spreadsheet.spreadsheetId) {
            console.error('[Scheduled Backup] Failed to create spreadsheet:', spreadsheet);
        ...
        console.log(`[Scheduled Backup] Created spreadsheet: ${spreadsheetId}`);
        ...
        console.log('[Scheduled Backup] Data written and formatted');
        ...
        console.log(`[Scheduled Backup] Moved spreadsheet to ${mainFolderName} folder`);

        const spreadsheetUrl = `https://docs.google.com/spreadsheets/d/${spreadsheetId}`;

        // שליחת מייל עדכון למנהל
        try {
            await base44.asServiceRole.integrations.Core.SendEmail({
                to: 'natib8000@gmail.com',
                subject: `גיבוי יומי אוטומטי ל-Google Sheets הושלם`,
                body: `
                    <div dir="rtl" style="font-family: Arial, sans-serif;">
                        <h2>גיבוי יומי אוטומטי הושלם!</h2>
